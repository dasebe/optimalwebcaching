...
uint64_t parseTraceFile(std::vector<traceEntry> & trace, std::string & path, uint64_t samples) {
    ...
    for(auto it: counts) {
        if(sampleCount + it.second > samples) {
            break;
        }
        sampleCount += it.second;
    ...
    while(traceFile >> time >> id >> size) {
        ...
        if(lastSeen.count(idSize)>0) {
            std::get<2>(trace[lastSeen[idSize]]) = true;
        } else {
            uniqc++;
        }
