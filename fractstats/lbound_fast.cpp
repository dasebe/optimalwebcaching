...
int main(int argc, char* argv[]) {
    ...
    for(size_t i=0; i<trace.size(); i++) {
        ...
        if(cur.nextSeen > 0) {
            ...
            utils.emplace_back(1.0/utilityDenominator, i);
        }
    }
...
#pragma omp simd
    ...
    // no parent
    size_t * noparent = new size_t[trace.size()];
#pragma omp simd
    for(size_t i=0; i<trace.size(); i++) {
        noparent[i] = 0;
    }
    ...
    std::unordered_set<size_t> noparentset;
    uint64_t counter = 0;
...
#pragma omp simd
        ...
        noparentset.clear();
        for(size_t i = 0; i<trace.size(); i++) {
            noparentset.insert(noparent[i]);
        }

        std::cout << "lboundfastc " + std::to_string(cs) + " " + std::to_string(cached.size()) + " " + std::to_string(totalReqc) +  " " + std::to_string((double)(cached.size())/totalReqc) + " " + std::to_string(noparentset.size()) + "\n";
    }
